...
st.write("""
    This application predicts the likelihood of diabetes based on patient data. Please log in to get started.
    """)
...
with st.sidebar:
    ...
    if st.session_state.current_user is None:
        ...
        st.subheader(f'Logged in as {st.session_state.current_user}')
        if st.button('Logout'):
            st.session_state.current_user = None
...
if predict_button:
    if st.session_state.current_user:
        ...
        with st.expander('Age vs Glucose'):
            ...
            df_healthy = df[df['diabetes'] == 0]
            ax_glucose.scatter(df_healthy['age'], df_healthy['blood_glucose_level'], c='blue', label='Healthy')
            ax_glucose.scatter(df_diabetes['age'], df_diabetes['blood_glucose_level'], c='red', label='Diabetic')
            ax_glucose.scatter(user_data['age'], user_data['blood_glucose_level'], c=color, s=150, label='Input Data')
            ax_glucose.set_xlabel('Age')
            ax_glucose.set_ylabel('Blood Glucose Level')
            ax_glucose.legend()
        ...
        with st.expander('Age vs Heart Disease'):
            ...
            ax_bp.scatter(df_healthy['age'], df_healthy['heart_disease'], c='blue', label='Healthy')
            ax_bp.scatter(df_diabetes['age'], df_diabetes['heart_disease'], c='red', label='Diabetic')
            ax_bp.scatter(user_data['age'], user_data['heart_disease'], c=color, s=150, label='Input Data')
            ax_bp.set_xlabel('Age')
            ax_bp.set_ylabel('Heart Disease')
            ax_bp.legend()
        ...
        with st.expander('Age vs Smoking History'):
            ...
            ax_st.scatter(df_healthy['age'], df_healthy['smoking_history'], c='blue', label='Healthy')
            ax_st.scatter(df_diabetes['age'], df_diabetes['smoking_history'], c='red', label='Diabetic')
            ax_st.scatter(user_data['age'], user_data['smoking_history'], c=color, s=150, label='Input Data')
            ax_st.set_xlabel('Age')
            ax_st.set_ylabel('Smoking History')
            ax_st.legend()
            plt.xticks(np.arange(10, 100, 5))
        ...
        with st.expander('Age vs Haemoglobin (HbA1c) Level'):
            ...
            ax_i.scatter(df_healthy['age'], df_healthy['HbA1c_level'], c='blue', label='Healthy')
            ax_i.scatter(df_diabetes['age'], df_diabetes['HbA1c_level'], c='red', label='Diabetic')
            ax_i.scatter(user_data['age'], user_data['HbA1c_level'], c=color, s=150, label='Input Data')
            ax_i.set_xlabel('Age')
            ax_i.set_ylabel('HbA1c Level')
            ax_i.legend()
            plt.xticks(np.arange(10, 100, 5))
        ...
        with st.expander('Age vs BMI'):
            ...
            ax_bmi.scatter(df_healthy['age'], df_healthy['bmi'], c='blue', label='Healthy')
            ax_bmi.scatter(df_diabetes['age'], df_diabetes['bmi'], c='red', label='Diabetic')
            ax_bmi.scatter(user_data['age'], user_data['bmi'], c=color, s=150, label='Input Data')
            ax_bmi.set_xlabel('Age')
        ...
        with st.expander('Age vs Hypertension'):
            ...
            fig_dpf, ax_dpf = plt.subplots()
            ax_dpf.scatter(df_healthy['age'], df_healthy['hypertension'], c='blue', label='Healthy')
            ax_dpf.scatter(df_diabetes['age'], df_diabetes['hypertension'], c='red', label='Diabetic')
            ax_dpf.scatter(user_data['age'], user_data['hypertension'], c=color, s=150, label='Input Data')
            ax_dpf.set_xlabel('Age')
            ax_dpf.set_ylabel('Hypertension')
            ax_dpf.legend()
            plt.xticks(np.arange(10, 100, 5))
            plt.yticks(np.arange(0, 2, 1))
            plt.title('0 - Healthy & 1 - Unhealthy')
        ...
        st.warning('Please log in before making predictions.')
